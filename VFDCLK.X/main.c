 ...
 * dígitos que mudarem (ver tela_principal). 0xFF = desconhecido, força a
 * primeira escrita.                                                  */
...
/* Tela principal — a LINHA SUPERIOR é SEMPRE o relógio; só a de baixo
 * alterna no carrossel:
 *   inferior == TELA_HORA :  |      14:35:27      |
 *                            |   QUA 16/07/2026   |
 *   inferior == TELA_CLIMA:  |      14:35:27      |
 *                            | 23.4 C     45.2 %RH|
 * Manter a hora fixa em cima é justamente o que faz a atualização
 * incremental dos segundos valer para as DUAS telas: o campo está
 * sempre na mesma posição (linha 0, coluna 12).                      */
static void tela_principal(bool completo, uint8_t inferior)
{
    ...
    if (inferior == TELA_CLIMA) {
        if (ha_medida) {
            por_decimos(&b1[4],  medida.temperatura_dC);
            por_decimos(&b1[15], (int16_t)medida.umidade_dRH);
        } else {
            b1[1]  = '-'; b1[2]  = '-'; b1[3]  = '.'; b1[4]  = '-';
